...
- 前端图片加载：瀑布流/列表/详情使用 `public_url` 的路径部分（`/images/{year}/{month}/{day}/{filename}`）展示，开发环境通过 Vite proxy 转发 `/images` 到后端；复制链接时统一补全 hostname，依赖年月日 + hash 作为直链保护。
- 前端开发：Vite dev/preview 默认绑定到 `0.0.0.0`，并在允许的 host 列表中加入 `memset0.cn`，保证非 localhost 环境也可直接访问调试。
- 详情页：移除“加载中...”文本，基于元数据尺寸展示等比例骨架；先显示缩略图，完整图加载完成后淡入替换，保持渐进式体验。
- 浏览页视图切换（瀑布流/列表）会重置分页到第一页并重新拉取数据，防止在另一视图滚动过多后切换产生卡顿。
- 收藏：images 表新增 is_favorite 字段；新增收藏/取消收藏接口 `/api/images/{id}/favorite` 与收藏列表 `/api/images/favorites`。
- 收藏：详情页增加收藏/取消收藏按钮；浏览页顶部新增 16:9 收藏轮播组件，无收藏则不展示，默认 5 秒切换并支持上一张/下一张操作，当前居中两侧半透明并带 3D 纵深过渡。
- 数据：缩略图表额外记录 size_bytes，生成缩略图时同步写入；编辑图片后会重置缩略图并刷新 size_bytes，列表视图可稳定显示原始文件名与大小。
- AI 标注：已接入 Qwen3 VL，同步调用 `/api/images/{id}/ai-analyze` 生成标签（source=ai）并入库，详情页提供“AI 生成标签”按钮。
- AI 标注：当生成失败时，详情页会展示后端返回的错误信息，便于定位配置/权限/额度问题。
- AI 标注：新增 `qwen.auto_tag_on_upload` 开关控制上传时自动生成标签，失败仅后端 warning，默认关闭。
- AI 检索：新增 `/api/mcp/search`，从数据库标签库中生成 prompt，让大模型选出 5 个标签并按重合数排序检索图片，前端新增“帮我找图”页面横向展示前 5 张并显示模型输出，等待时展示搜索动画。
...
1. MCP 接口：已实现“帮我找图”文本检索，后续可扩展多模态对话检索
