...
- 前端：React + MUI + React Query；瀑布流响应式（1/2/3 列，列宽固定、按最矮列落位，缩略图加载即 Grow 出现，完整图加载后无缝覆盖，列数变更时重新按顺序分配并动画展示）；上传提供拖拽/多文件并行与复制直链。
...
9. 浏览页瀑布流：列宽固定的 1/2/3 列响应式布局，图片保持原始宽高比并按加载顺序落到当前总高度最小的列；缩略图加载即通过 Grow 展示，完整图加载后替换缩略图；列数变更时按顺序重排并动画展示，滚动加载 20 张
