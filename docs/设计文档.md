...
- 前端：React + MUI + React Query；瀑布流响应式（1/2/3/4/5 列，列宽固定、按最矮列落位且同高取最左、分配顺序固定保证刷新稳定；缩略图加载即 Grow 出现，完整图加载后无缝覆盖，列数变更时重新按顺序分配并动画展示），支持列表视图切换（表格背景透明）；上传提供拖拽/多文件并行、Ctrl+V 粘贴图片上传与复制直链。
